...
def prefix_key(root_id: int, step: int):
    ...
    return prng_key(f"ea34:prefix:{root_id}:{step}")


def probe_key(root_id: int, step: int):
    from execution_aligned_rl.v3.policy import prng_key

    return prng_key(f"ea34:probe:{root_id}:{step}")
...
def worker_main(args) -> None:
    ...
    sentinel_ids = {720000, 720001, 720002, 720003, 720004}
    sentinels = [b for b in legal if b["manifest"]["root_id"] in sentinel_ids]
    # A-pool-A: D3 again after the rest of the pool (primary already ran in order)
    from execution_aligned_rl.v3.runtime_qual.compare_v2 import compare_traces_v2 as _cmp
    apool = []
    for bundle in sentinels:
        rec_after = run_root(bundle, "apool_after")
        # compare with in-memory? reload primary later in finalize. Record after traces only.
        apool.append({"root_id": rec_after["root_id"], "d0_after": rec_after["d0"]["status"]})
        print({"apool_after": rec_after["root_id"], "d0": rec_after["d0"]["status"]})
    dump_json(dest / "apoola.json", {"sentinels": apool})
